...
\title{Solutions for Chapter 1}
    ...
    \ex{1.8}
    A $50 \mu \A$ meter movement has an internal resistance of 5k. What shunt resistance is needed to convert it to a $0 - 1 \A$ meter? What series resistance will convert it to a $0 - 10 \V$ meter?
    \begin{enumerate}
        \item 
        A small resistance ($R_1$) in parallel would be expected to the movement with internal resistance of 5K. So showing the same in the figure below.
        
            \begin{circuit}{fig:1.8.1}{Voltage divider with $R_2$ and $R_\load$ in parallel}
                % \label{1.8.1fig1}
                (0,0) to[I=$I_0$] (0,3)
                to[short, -*, i=$1 A$] (2,3)
                to[R=$R_1$, i>_=$I_1$] (2,0) -- (0,0);
                \draw (2,3) -- (4,3)
                to[R=$5 k\Ohm $, i>_=$50 \mu \A$]
                (4,0) to[short, -*] (2,0);
            \end{circuit}
        
        In this circuit the 1 A reading $R_1$ and movement resistance will have the same voltage across them.
        
        Hence By Ohm's law  
                        \[{R_{1}\cdot I{_1}} = {50 \mu \A\cdot 5\k\Ohm}\] 
        
                        \[I_{1} = 1 \A - 50 \mu \A \]
                        
                        \[R_{1} = \frac{50 \mu \A\cdot 5\k\Ohm}{1 \A - 50 \mu \A}\]
                        
        Hence
                        \[R_{1} = \frac{5 \cdot 10^{-6} \cdot 5 \cdot 10^{3} }{0.99} = \mans{0.03 \Ohm} \]
                        
                
    \end{enumerate}
    
    \ex{1.10}
    For the circuit shown in Figure 1.12 , with $V_\in= 30 V$ and $R_1=R_2=10k$, find (a) the output voltage with no load attached (the open-circuit voltage); (b) the output voltage with a 10k load (treat as a voltage divider, with $R_2$ and $R_\load$ combined into a single resistor); (c) the Thevenin equivalent circuit; (d) the same as in part (b), but using the Thevenin equivalent circuit [again, you wind up with a voltage divider; the answer should agree with the result in part (b)]; (e) the power dissipated in each
    \begin{enumerate}
        \item 
        ...
        \[V_{out} = V_{in} \frac{5\k\Ohm}{10\k\Ohm + 5\k\Ohm} = \frac{30\V}{3} = \mans{10\V} \]
          ...
          \begin{circuit}{fig:1.10.1}{Voltage divider with $R_2$ and $R_\load$ in parallel}
            % \label{1.10fig1}
            (0,0) to[V=$V_{\in}$] (0,2)
            to[R=$R_1$](2,2)
            to[R=$R_2$] (2,0)
        ...
        \begin{circuit}{fig:1.10.4}{Th\'evenin equivalent circuit.}
            ...
            (3,0) to[open, v_<=$V_\out$] (3,2)
        \end{circuit}
        In terms of behavior at the ports, this circuit is equivalent to the circuit in Figure \ref{fig:1.10.2}. 

        \item 
        We connect the $10\k\Ohm$ load to the port of the Th\'evenin equivalent circuit in Figure \ref{fig:1.10.4} to get the following circuit.
